<script lang="ts">
  ...
  async function checkContentExists() {
    try {
      const response = await fetch(`/api/check-content?brand=${encodeURIComponent(brand)}&model=${encodeURIComponent(model)}&year=${year}`);
      if (response.ok) {
  ...
  {:else}
    ...
    <div class="text-center py-12">
      ...
      <p class="text-gray-500 dark:text-gray-400 mb-6">
        Generate AI-powered detailed information about the {brand} {model}.
      </p>
      <button 
        on:click={generateContent}
        class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors"
